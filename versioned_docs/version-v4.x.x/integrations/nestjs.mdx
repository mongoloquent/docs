...
```bash
npm install @mongoloquent/nestjs @mongoloquent/core mongodb
```
...
<Tabs>
    ...
    ```typescript
    // users.model.ts
    import { Model, IMongoloquentSchema, IMongoloquentTimestamps } from "@mongoloquent/core"
...
The `@Transactional` decorator accepts two optional parameters:
  - `connectionName`:  The name of the MongoDB connection as configured in `forRoot` or `forRootAsync`.
  - `retries`: The number of times the `transaction` should be retried if it fails due to transient errors.

> **Notice**: The `@Transactional` decorator manages the transaction lifecycle automatically, committing if the method completes successfully and rolling back if an error is thrown.

> **Important**: Just like the `callback` approach, the Mongoloquent `DB` class's `transactions` only work within the same `connection`.
...
You can find the API reference for the `@mongoloquent/nestjs` package [here](https://api.nestjs.mongoloquent.com).
